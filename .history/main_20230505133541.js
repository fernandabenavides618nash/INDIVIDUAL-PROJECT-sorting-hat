...
const filter = () => {
  ...
  const filterS = document.getElementById('#btnSlytherin');
  ...
  filterH.addEventListener('click', () => {
    ...
    allCards.forEach(card => {
      if(!card.classList.contains('colorYellow')){
        card.crea
      }
    });
...
const createStudentCard = (name, house, houseColor) => {
  const domString = `<div hidden="false" class="card card-school w-75 color${houseColor}" id="card${name}">
  <div class="card-body">
